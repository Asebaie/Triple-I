...
def mol_descriptors(mol):
    if mol is None: return np.zeros(22, dtype=np.float32)
    try:
        hbd = Descriptors.NumHDonors(mol)
        hba = Descriptors.NumHAcceptors(mol)
        return np.array([
            Descriptors.MolWt(mol), Descriptors.MolLogP(mol), hbd, hba,
            hbd + hba, hbd * hba, Descriptors.TPSA(mol),
...
def featurize_pair(smi1, smi2):
    ...
    sims = np.array([
        ...
        tanimoto(rdk1,    rdk2),
        d1[2]*d2[3], d2[2]*d1[3], (d1[2]+d2[2])*(d1[3]+d2[3]),
        min(d1[0],d2[0])/max(d1[0],d2[0]) if max(d1[0],d2[0])>0 else 0,
        abs(d1[1]-d2[1]), d1[6]+d2[6],
    ...
    return np.concatenate([
        ecfp4_1, ecfp4_2, ecfp4_1*ecfp4_2, np.abs(ecfp4_1-ecfp4_2),
        ecfp6_1, ecfp6_2, ecfp6_1*ecfp6_2, np.abs(ecfp6_1-ecfp6_2),
        maccs1,  maccs2,  maccs1*maccs2,
        rdk1,    rdk2,    rdk1*rdk2,
        d1, d2, np.abs(d1-d2), d1+d2,
        sims, extra,
    ])
...
def main():
    ...
    params = {
        ...
        'learning_rate': 0.015, 'num_leaves': 511, 'max_depth': -1,
        'min_child_samples': 8, 'feature_fraction': 0.45,
        'bagging_fraction': 0.75, 'bagging_freq': 5,
        'scale_pos_weight': spw, 'reg_alpha': 0.15, 'reg_lambda': 0.8,
        'min_split_gain': 0.005, 'verbose': -1, 'seed': SEED, 'n_jobs': -1,
    }
